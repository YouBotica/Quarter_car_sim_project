...
function [x_array] = qc_time_nonlinear_simulation(t0, x0, steps, dt, road_input, params)
    ...
    for iter = 1:steps
        ...
        ddx_s = (1/ms)*(Ks*(spring_deflection) + Cs*(x(4) - x(2))*(x(4) - x(2))*sign(x(4) - x(2)));
        ddx_u = (1/mu)*(Kt*(road_input(iter) - x(3)) - Ks*(spring_deflection) - Cs*(x(4) - x(2))*(x(4) - x(2))*sign(x(4) - x(2)));
        x(2) = x(2) + dt*ddx_s;
        x(4) = x(4) + dt*ddx_u;
        x(1) = x(1) + dt*x(2);
        x(3) = x(3) + dt*x(4);
